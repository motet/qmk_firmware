...
static void animate_waves(void) {
	// only 3 frames for last wave type
...
static void setup_stars(void) {
	...
	// 36% probability it will not be added (said another way, 80% chance it will start out lit in the x direction, 
	// then 80% chance it will start out lit in the y direction = 64% probability it will start out lit at all)
...
static void setup_shooting_star(struct ShootingStar *shooting_star) {
	// Shooting_stars travel diagonally 1 down, 1 across.
	// So the lowest a shooting_star can start and not 'hit' the ocean is 32 above the ocean.
...
static void animate_shooting_stars(void) {
	...
	if (number_of_shooting_stars == 0) {
		end_extra_stars(0);		// make sure all shooting_stars are ended
	} else {
...
void render_stars(void) {
	...
	// fix for OLED flicker and on/off bug
	if ((current_wpm > 0) || (last_input_activity_elapsed() < OLED_TIMEOUT) || (last_led_activity_elapsed() < OLED_TIMEOUT)) {
		oled_on();
